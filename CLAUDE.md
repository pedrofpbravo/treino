...
Architecture notes for future sessions. This describes the app as it actually is (v7.1).
...
- **Treino**: pick a program (dropdown + Gerenciar sheet) and a day (chips). Day chips show a "✓" for days trained in the current program cycle, and that comes ONLY from an explicit finished session (`cycleProgress()`, see Key behaviors). Each exercise is a compact uniform-height card: name + target pill ("3×12"), a "Ref: <kg>" line that also carries the state badge on its right (`✓ feito`, or `2/3 séries` while in progress), one-line note. There is NO checkbox on the card (removed in v7: tapping it to expand kept deleting the log). Since v7.1 a workout in progress is a LOCAL DRAFT (`gym:drafts` in localStorage, keyed `"date|dayId"` per exercise), never a Firestore write: tapping the card body starts the exercise by creating its draft, pre-filled purely from the day target (`prefillSets(entry, refWeight)`: targetSets rows of the target reps at the exercise's reference weight; the last session never feeds the prefill — if today's log already exists, e.g. finished + reopened, the draft seeds from it via `draftFromLog`). The inline sets editor (kg-first per set, per-set check toggle, add/remove set) mutates the draft and re-saves it to localStorage only. Tapping a started card collapses/expands it. Checking a set auto-starts the 90s rest timer; checking the LAST pending set auto-collapses the card and marks it done. Card state comes from the draft: peach fill + `✓ feito` only when ALL sets are checked (`draftAllDone`), accent left edge only with ≥1 checked set, and a merely opened card (0 checked) looks idle with a `0/N séries` badge — expanding/collapsing never changes color and never records anything. Drafts from previous dates are pruned on render: an unfinished workout is discarded, by design. **Nothing on the Treino tab deletes a log**: removal happens only in Histórico (swipe). "Finalizar treino" is the ONLY moment logs are written: one log per exercise with ≥1 checked set, containing the checked sets only (0-checked exercises stay out of the history), then the finished session — all fire-and-forget so it works offline; the draft is kept for the rest of the day, so cards stay painted and re-finishing after a reopen overwrites the same deterministic log ids. It records the session even when exercises remain incomplete; while a workout is open, a sticky "Treino em andamento · Finalizar" bar appears once that button scrolls out of view (IntersectionObserver). Long-press (~350ms) a card to drag-reorder the day's exercises (persists entry order via updateDay). The ⚙ gear opens the quick-detail sheet (day target + refWeight/note + a "Histórico" link row that opens the full exercise log). "＋ Cardio" (always visible, standalone) opens a sheet: type dropdown, whole minutes, note; today's cardio entries list below with a remove ×. Rest timer presets (60/90/120s) live inline AND inside the floating bar; the bar (Treino tab only) counts down and at zero just blinks at 0:00 until dismissed (X, a new preset, or checking exercise/set). No sound, no vibration (both removed in v7). A running timer survives reloads; a finished one doesn't.
- **Exercícios**: the full catalog grouped by primary muscle, accent-insensitive search, muscle-chip filters (toggling a filter resets the page scroll to top — v7.1, otherwise the shrunken list left the viewport clamped mid-air and read as a broken layout). Tapping opens the full edit sheet (name, primary/secondary/other muscles, refWeight, note, delete, plus a "Histórico" link row on existing exercises).
...
- `logs/log-<date>-<dayId>-<exerciseId>`: `{date "YYYY-MM-DD" LOCAL, programId, dayId, exerciseId, exerciseName, dayName, programName, sets: [{reps, weight|null, done}], ts}`. Since v7.1, written ONLY by "Finalizar treino" and carrying the checked sets only (all `done: true`); the deterministic id makes re-finishing an overwrite, not a duplicate. Name snapshots keep history working after deletions. Logs written before v6 have no `done` per set; `cloneSets()`/`draftFromLog()` treat a missing flag as done (no migration).
...
- Starting an exercise pre-fills purely from the day entry's target: targetSets rows of `reps`, weight = `parseRefWeight(refWeight)`. The last session NEVER changes the prefill (v7.1; it lives only in the history) — adjustments happen by editing inside the card or the ⚙ sheet. Nothing is saved to Firestore until "Finalizar treino"; completion is per-set inside the local draft.
...
- localStorage keys: `gym:program`, `gym:day`, `gym:timerEnd`, `gym:collapsed` (collapsed workout cards, keyed `dayId|exerciseId`), `gym:workoutStart` (`<date>|<dayId>` of a workout started by tapping the button), `gym:drafts` (v7.1: the in-progress workout, `{"date|dayId": {exerciseId: sets[]}}`, pruned to today on every Treino render — the ONE localStorage key holding real workout data until "Finalizar treino" flushes it to Firestore), `gym:cycle-celebrated:<programId>` (last celebrated cycle key), plus one-time flags `gym:migrate-refweight-v6`, `gym:seed-smith-v6` (UI prefs/flags only otherwise; all recorded data lives in Firestore).
