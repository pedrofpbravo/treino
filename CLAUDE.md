...
- `cardioTypes/ct-<key>`: `{name, order}` (seeded: bike, eliptico, esteira, escada, corrida, remo, outro; managed in Ajustes like muscles).
...
- One-time boot upserts guarded by localStorage flags: refWeight migration and the `ex-agachamento-smith` catalog add (`gym:seed-smith-v6`).
